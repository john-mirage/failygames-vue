...
import {MenuIcon, SunIcon, MoonIcon, XIcon} from "@heroicons/vue/solid";
    ...
    <div class="container px-6 mx-auto flex flex-row items-center h-full">
      <div class="flex-1 flex flex-row justify-start">
        <Popover class="relative" v-slot="{ open }">
          <PopoverButton>
            <span class="flex w-12 h-12 rounded-full cursor-pointer -ml-3">
              <MenuIcon v-if="!open" class="h-6 w-6 m-auto text-slate-500 dark:text-slate-50"/>
              <XIcon v-else class="h-6 w-6 m-auto text-slate-500 dark:text-slate-50" />
            </span>
          </PopoverButton>
          <transition
